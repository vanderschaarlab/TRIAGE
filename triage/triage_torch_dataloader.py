#
# VERSION OF TRIAGE TORCH USING A DATALOADER
#
...
class Triage:
    def __init__(self, eval_dataloader, cal_dataloader, learner):
    ...
    def compute_pred_metrics(self, iteration=10, knn=True):
        ...
        # go through the cal loader and get the predictions & then compute the residuals, finally storing as numpy
        self.learner.eval()
        residuals_cal = []
        X_cal_list = []
        for X_cal, y_cal in self.cal_dataloader:
            y_hat_cal = self.learner(X_cal)
            y_hat_cal = y_hat_cal.detach().cpu().numpy().reshape(-1)
            y_cal = y_cal.detach().cpu().numpy().reshape(-1)
            y_cal, y_hat_cal = np.squeeze(y_cal), np.squeeze(y_hat_cal)
            residuals_cal.append(y_cal - y_hat_cal)
            X_cal_list.append(X_cal.detach().cpu().numpy())

        residuals_cal = np.concatenate(residuals_cal)
        residuals_cal = residuals_cal.reshape(-1, 1)
        self.X_cal = np.concatenate(X_cal_list)

        # go through the eval loader and get the predictions
        pred_eval = []
        X_eval_list = []
        y_eval_list = []
        for X_eval, y_eval in self.eval_dataloader:
            pred_eval.append(self.learner(X_eval).detach().cpu().numpy().reshape(-1))
            X_eval_list.append(X_eval.detach().cpu().numpy())
            y_eval_list.append(y_eval.detach().cpu().numpy())

        # flatten the list of predictions
        y_hat_eval = np.concatenate(pred_eval)

        # flatten the list of X_eval
        self.X_eval = np.concatenate(X_eval_list)
        self.y_eval = np.concatenate(y_eval_list)
    ...
    def compute_scoring_metrics(self, compute_cpd=False, compute_crps=False):
        ...
        residuals_cal = np.squeeze(residuals_cal)
        ...
        triage_score = cps_norm.predict(y_hat=y_hat, sigmas=sigmas, y=self.y_eval)

        print("got triage score")
